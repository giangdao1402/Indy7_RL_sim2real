...
class IndyReachPolicy(PolicyController):
    """Policy controller for Indy Reach using a pre-trained policy model."""
    ...
    def __init__(self, robot_ip) -> None:
        ...
        self.load_policy(
            model_dir / "policy_advanced.pt",
            model_dir / "env_advanced.yaml",
        )
        ...
        self.prev_joint_pos = np.zeros(self.num_joints, dtype=np.float32)
        self.prev_joint_vel = np.zeros(self.num_joints, dtype=np.float32)
        # Histories (2-step)
        self.joint_pos_history = [np.zeros(self.num_joints, dtype=np.float32) for _ in range(2)]
        self.joint_vel_history = [np.zeros(self.num_joints, dtype=np.float32) for _ in range(2)]
        self.action_history = [np.zeros(self.num_joints, dtype=np.float32) for _ in range(2)]
    ...
    def _compute_observation(self, command: np.ndarray) -> np.ndarray:
        ...
        pos_hist = np.array(self.joint_pos_history, dtype=np.float32).flatten()
        vel_hist = np.array(self.joint_vel_history, dtype=np.float32).flatten()
        action_hist = np.array(self.action_history, dtype=np.float32).flatten()
    ...
    def forward(self, dt: float, command: np.ndarray) -> np.ndarray:
        ...
        if self._policy_counter % self._decimation == 0:
            ...
            print(f"{'Command:':<20} {np.round(obs[2*self.num_joints:2*self.num_joints+7], 4)}")
            print(f"{'Pos Hist:':<20} {np.round(np.array(self.joint_pos_history).flatten(), 4)}")
            print(f"{'Vel Hist:':<20} {np.round(np.array(self.joint_vel_history).flatten(), 4)}")
            print(f"{'Action Hist:':<20} {np.round(np.array(self.action_history).flatten(), 4)}")
            print("--- Action ---")
...
if __name__ == "__main__":
    ...
    policy = IndyReachPolicy(ROBOT_IP)
